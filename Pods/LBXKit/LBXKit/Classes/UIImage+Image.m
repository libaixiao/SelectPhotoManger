...
- (void)compressedImageKB:(CGFloat)fImageKBytes imageBlock:(void (^)(NSData *))block{
    //二分法压缩图片
...
- (void)resetSizeimageKB:(CGFloat)fImageKBytes imageBlock:(ImageBlock)block{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
           ...
           UIImage *newImage = [self createImageForData:imageData maxPixelSize:MAX((NSUInteger)self.size.width, (NSUInteger)self.size.height)];
