...
{
    objc_setAssociatedObject(self, @selector(_addActionHandler:), touchHandler, OBJC_ASSOCIATION_COPY_NONATOMIC);
...
{
    TouchedButtonBlock block = objc_getAssociatedObject(self, @selector(_addActionHandler:));
    if (block)
    {
        block(btn);
    }
